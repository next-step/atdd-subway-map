...
import nextstep.subway.error.exception.LengthOfLineIsOneException;
...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ErrorResponse> handleSectionAlreadyHasStationException(SectionAlreadyHasStationException ex) {
        ErrorResponse errorResponse = new ErrorResponse(ex.getErrorCode(), ex.getErrorMessage());
        return new ResponseEntity(errorResponse, HttpStatus.valueOf(ex.getErrorCode().getStatus()));
    }

    @ExceptionHandler(LengthOfLineIsOneException.class)
    public ResponseEntity<ErrorResponse> handleLengthOfLineIsOneException(LengthOfLineIsOneException ex) {
        ErrorResponse errorResponse = new ErrorResponse(ex.getErrorCode(), ex.getErrorMessage());
        return new ResponseEntity(errorResponse, HttpStatus.valueOf(ex.getErrorCode().getStatus()));
    }
}
