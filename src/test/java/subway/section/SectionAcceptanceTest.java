...
class SectionAcceptanceTest extends ApiTest {
    ...
    @Test
    void addSectionExceptionIsINVALID_UP_STATION() {
        // given : 선행조건 기술
        당고개역부터_이수역까지의_기존_노선_생성();
     ...
     * When 기존 구간(2. 이수역 - 상행 종점, 사당역 - 하행 종점))을 제거한다.
    ...
    void removeSection() {
        ...
        // when : 기능 수행
        ExtractableResponse<Response> response = 지하철_노선_구간_제거(당고개역부터_이수역까지의_기존_노선_ID);
    ...
    private void 이수역부터_사당역까지의_신규노선_생성(Long 당고개역부터_이수역까지의_기존_노선_ID) {
        long 신규_노선_ID = 지하철노선등록요청(지하철노선등록요청_생성("4호선", "bg-blue-500", 이수역, 사당역, 10)).jsonPath().getLong("id");
        ...
        assertThat(신규_노선_ID).isEqualTo(2L);
        assertThat(이수역부터_사당역까지의_신규구간_생성.statusCode()).isEqualTo(HttpStatus.OK.value());
    }

    private static ExtractableResponse<Response> 지하철_노선_구간_제거(Long 당고개역부터_이수역까지의_기존_노선_ID) {
        return RestAssured.given().log().all()
                .param("stationId", 2L)
