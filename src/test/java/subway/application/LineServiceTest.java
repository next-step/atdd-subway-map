...
class LineServiceTest {
    ...
    void createLine() {
        ...
        final Section 첫번째_구간 = new Section(상행종점역, 하행종점역, 10);
        final Line 노선 = new Line(1L, "2호선", "bg-red-600", 구간_신규_등록(첫번째_구간));

        when(stationService.findById(anyLong())).thenReturn(상행종점역)
                .thenReturn(하행종점역);
        when(lineRepository.save(any())).thenReturn(노선);
    ...
    void showLines() {
        ...
        final Section 이호선_첫번째_구간 = new Section(상행종점_강남역, 하행종점_잠실역, 10);
        final Line 이호선 = new Line(1L, "2호선", "bg-red-600", 구간_신규_등록(이호선_첫번째_구간));

        final Station 상행종점_양재역 = new Station(3L, "양재역");
        final Station 하행종점_몽총토성역 = new Station(4L, "몽촌토성역");
        final Section 신분당선_첫번째_구간 = new Section(상행종점_양재역, 하행종점_몽총토성역, 10);
        final Line 신분당선 = new Line(2L, "신분당선", "bg-green-600", 구간_신규_등록(신분당선_첫번째_구간));
