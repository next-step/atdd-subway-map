...
public class LineAcceptanceTest {
    ...
    @DisplayName("해당 노선의 하행 종점이 아닌 역을 상행선으로 가지는 구간 추가시 실패한다.")
    @Test
    ...
    @DisplayName("해당 노선에 이미 등록된 역을 하행선으로 가지는 구간 추가시 실패한다.")
    @Test
    ...
    void deleteSection() {
        ...
        // when
        지하철_구간_제거_요청(stationResponse.getId());
    ...
    @DisplayName("하행 종점역이 아닌 역을 제거 요청시 실패한다.")
    @Test
    void deleteSection_fail_sectionDoesNotMatchWithDownEndStationOfLine() {
        ...
        // when
        지하철_구간_제거_요청(getUpEndStationId(lineCreateResponse));
    ...
    // Then 지하철 노선 조회시 추가한 구역이 제거되지 않은 것을 확인할 수 있다.
    @DisplayName("상행 종점역과 하행 종점역만 있는 노선에 구간 삭제 요청시 실패한다.")
    @Test
    void deleteSection_fail_sectionOnlyExistsUpAndDownEndStationInLine() {
        ...
        // when
        var statusCode = 지하철_구간_제거_실패_요청_상태_코드_반환(lineCreateResponse.getId(), getUpEndStationId(lineCreateResponse));
