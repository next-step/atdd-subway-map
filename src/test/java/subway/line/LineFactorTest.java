...
public class LineFactorTest {
    ...
    /**
     * Given 지하철 노선을 생성하고
     * When 구간상행역이 노선종점역이 아니게 등록할 경우
     * Then 해당 지하철 노선 정보에 구간이 추가가 실패된다.
     */
    @DisplayName("지하철 노선 추가 후 구간상행역이 노선 종점역이 아니게 등록할 경우 실패되어야 한다.")
    @Test
    void 지하철노선_구간추가_종점역이_상행역이_아닐경우_추가실패() {
        // given
        Long id = 지하철노선_생성_후_식별값_리턴(SHINBUNDANG_LINE_NAME, SHINBUNDANG_LINE_COLOR, GANGNAM_STATION_NAME, SEOLLEUNG_STATION_NAME, 10);

        // when
        ExtractableResponse<Response> response = 지하철노선_구간_추가(id, NOWON_STATION_NAME, SEOLLEUNG_STATION_NAME, 3);

        // then
        지하철노선_구간추가_종점역이_상행역이_아닐경우_추가실패_응답값_검증(response);
    }

    /**
     * Given 지하철 노선을 생성하고
     * When 이미 노선에 등록된 하행역을 구간하행역으로 등록하게될경우
     * Then 해당 지하철 노선 정보에 구간이 추가가 실패된다.
     */
    @DisplayName("지하철 노선 추가 후 구간하행역이 노선에 이미 등록되어있으면 실패되어야 한다.")
    @Test
    void 지하철노선_구간추가_하행역이_이미등록되어있으면_추가실패() {
        // given
        Long id = 지하철노선_생성_후_식별값_리턴(SHINBUNDANG_LINE_NAME, SHINBUNDANG_LINE_COLOR, GANGNAM_STATION_NAME, SEOLLEUNG_STATION_NAME, 10);

        // when
        ExtractableResponse<Response> response = 지하철노선_구간_추가(id, SEOLLEUNG_STATION_NAME, GANGNAM_STATION_NAME, 3);

        // then
        지하철노선_구간추가_하행역이_이미등록되어있으면_추가실패_응답값_검증(response);
    }
    ...
    private void 지하철노선_구간추가_종점역이_상행역이_아닐경우_추가실패_응답값_검증(ExtractableResponse<Response> response) {
        assertThat(response.statusCode()).isEqualTo(HttpStatus.CONFLICT.value());
        assertThat(response.asString()).isEqualTo("구간정보에 상행역이 현재 노선에 하행 종점역이 아닙니다.");
    }

    private void 지하철노선_구간추가_하행역이_이미등록되어있으면_추가실패_응답값_검증(ExtractableResponse<Response> response) {
        assertThat(response.statusCode()).isEqualTo(HttpStatus.CONFLICT.value());
        assertThat(response.asString()).isEqualTo("구간 하행역이 이미 노선에 등록되어 있습니다.");
    }
