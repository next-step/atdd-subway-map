...
public class SectionAcceptanceTest extends AcceptanceTest {
     ...
     * given   : backgound 에 맞춰 구간을 생성하고
     * when    : 강남역을 하행역으로 등록하여 구간을 생성하면
     * then    : 400 에러가 반환된다.
     */
    @DisplayName("구간 등록시 존재하는 역 하행으로 등록")
    @Test
    void 존재하는_역_하행으로_등록() {
        // given
        구간_생성_요청(강남역Id, 역삼역Id, 강남_역삼_거리, 신분당선Id);
        ...
        // when
        ExtractableResponse<Response> postResponse = 구간_생성_요청(역삼역Id, 강남역Id, 강남_역삼_거리, 신분당선Id);
     ...
     * then    : 400 에러가 반환된다.
     */
    @DisplayName("구간 등록시 거리를 비정상적으로 입력")
    @Test
    void 거리를_비정상으로_입력() {
        // given
        구간_생성_요청(강남역Id, 역삼역Id, 강남_역삼_거리, 신분당선Id);
        ...
        // when
        ExtractableResponse<Response> postResponse = 구간_생성_요청(역삼역Id, 선릉역Id, 역삼_선릉_거리, 신분당선Id);
     ...
     * then    : 400 에러가 반환된다.
     */
    @DisplayName("구간 삭제시 최하행역이 아니면 에러")
    @Test
    void 최하행역이_아닌_역_구간_삭제() {
        ...
        Long 선릉역Id = extractId(역_생성("선릉역"));
        int 역삼_선릉_거리 = 5;
        구간_생성_요청(역삼역Id, 선릉역Id, 역삼_선릉_거리, 신분당선Id);

        // when
        ExtractableResponse<Response> deleteResponse = 구간_삭제_요청(신분당선Id, 역삼역Id);
