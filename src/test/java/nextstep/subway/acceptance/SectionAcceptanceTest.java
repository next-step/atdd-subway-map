...
public class SectionAcceptanceTest {
    ...
    void setUp() {
        ...
        지하철역_ID = 지하쳘역_등록됨("지하철역");
        다른지하철역_ID = 지하쳘역_등록됨("다른지하철역");
        또다른지하철역_ID = 지하쳘역_등록됨("또다른지하철역");
        노선_ID = 노선_생성됨("신분당선", "bg-red-600", 지하철역_ID, 다른지하철역_ID, 10);
    }

    /**
     * When 지하철 구간 등록 요청을 하면
    ...
    void createSection() throws Exception {
        // when
        final ExtractableResponse<Response> 지하철_구간_등록_응답 = 지하철_구간_생성_요청(노선_ID, 다른지하철역_ID, 또다른지하철역_ID, 10);

        // then
        요청이_정상적으로_이루어졌는지_확인(지하철_구간_등록_응답, CREATED);
        assertThat(지하철_구간_등록_응답.jsonPath().getLong("upStationId")).isEqualTo(다른지하철역_ID);
        assertThat(지하철_구간_등록_응답.jsonPath().getLong("downStationId")).isEqualTo(또다른지하철역_ID);
    ...
    /**
     * When 지하철 구간 등록 요청을 하면
     * Then 새로운 구간의 상행역과 기존 노선의 하행 종점역이 일치하지 않는 예외 처리가 발생합니다.
     */
    ...
    void createSectionErrorAboutUpStationAndDownStationIsNotMatched() throws Exception {
        // when
        final ExtractableResponse<Response> 지하철_구간_등록_응답 = 지하철_구간_생성_요청(노선_ID, 지하철역_ID, 또다른지하철역_ID, 10);
    ...
    /**
     * When 지하철 구간 등록 요청을 하면
     * Then 새로운 구간의 하행역이 기존 노선에 이미 존재하는 예외 처리가 발생합니다.
     */
    ...
    void createSectionErrorAboutNewDownStationIsAlreadyExists() throws Exception {
        // when
        final ExtractableResponse<Response> 지하철_구간_등록_응답 = 지하철_구간_생성_요청(노선_ID, 다른지하철역_ID, 지하철역_ID, 10);
     ...
     * Then 지하철 노선에 대한 구간 목록에서 조회되지 않습니다.
     */
    @DisplayName("지하철 구간 삭제")
    @Test
    void deleteSection() throws Exception {
        // when
        final Long 생성된_하행_종점역_ID = 지하철_구간_생성됨(노선_ID, 다른지하철역_ID, 또다른지하철역_ID, 10);
    ...
    /**
     * When 지하철 구간 삭제를 요청하면
     * Then 삭제하려는 구간이 모든 구간 중, 마지막 구간이 아니라는 예외 처리가 발생합니다.
     */
    @DisplayName("지하철 구간 삭제 시, 삭제하려는 구간이 마지막 구간이 아닐 경우")
    @Test
    void deleteSectionErrorAboutIsNotALastSection() throws Exception {
        // when
        지하철_구간_생성됨(노선_ID, 다른지하철역_ID, 또다른지하철역_ID, 10);
        final ExtractableResponse<Response> 지하철_구간_삭제_응답 = 지하철_구간_삭제_요청(노선_ID, 다른지하철역_ID);
