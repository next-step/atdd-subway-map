...
- [ ] 추후 노선에 종속된 기능에 대한 인수테스트 작성 시 기능별로 분리한 메서드를 클래스를 분리하여 재사용성을 높일 수 있다.
...
- [ ] 노선 구간 등록
  - [ ] 새 구간의 상행역은 노선에 등록되어 있는 하행 종점역이 된다.
...
### 인수 조건
- [ ] 노선 구간 등록
  - when 1개의 지하철 구간을 등록하면
  - then 해당 지하철 노선 조회 시 생성한 구간 전체를 볼 수 있다.
- [ ] 이미 등록된 구간 등록 예외 검증
